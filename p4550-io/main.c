...
/**
 * The following program reads data on PORTB and shows it on PORTA and PORTE
 * First six LSB bits are shown on PORTA0:5
 * Last two bits are shown on PORTE0:1
 */
void main(void) {
    OSCCON = 0x79;
    ...
    while (1) {
        PORTA = ~(PORTB | 0xC0);
        PORTCbits.RC0 = !PORTBbits.RB6;
        PORTCbits.RC1 = !PORTBbits.RB7;
    }
}
